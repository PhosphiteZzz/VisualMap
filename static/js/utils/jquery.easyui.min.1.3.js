...
(function($) {
	function _7e(_7f) {
		...
		if (_81.showPageList) {
			...
			$("<td></td>").append("<span>每页</span>").appendTo(tr);
			$("<td></td>").append(ps).appendTo(tr);
			$("<td></td>").append("<span style=\"padding-right:6px;\">条记录</span>").appendTo(tr);
...
(function($) {
	...
	function _49c(_49d) {
		...
		var _49e = wrap.width()-4;
		var _49f = wrap.height()-4;
		var view = dc.view;
		var _4a0 = dc.view1;
		var _4a1 = dc.view2;
	...
	function _4c9(_4ca) {
		...
		function _4cf(_4d4, _4d5, _4d6) {
			...
			var t = $(
					...
					.appendTo(_4d4);
			//多行表头边框控制CLASS @zhulin
			var multiLine = "personality-singleLine";
			if(_4d5.length>1){
				multiLine = "personality-multiLine";
			}
			for ( var i = 0; i < _4d5.length; i++) {
				var tr = $("<tr class=\"datagrid-header-row "+multiLine+" \"></tr>").appendTo(
						$("tbody", t));
				var cols = _4d5[i];
				for ( var j = 0; j < cols.length; j++) {
					...
					if (col.checkbox) {
						...
						} else if (col.headerWarp) {
							$("<div class=\"datagrid-cell-group\" style=\"height: 50px\"></div>")
	...
	function _852(_853) {
		...
		function _855() {
			...
			_858.find(".datebox-current").click(function() {
				var _857 = opts.formatter(new Date());
				_85b(_853, _857);
				$(_853).combo("hidePanel");
				opts.onSelect.call(_853, new Date());
			});
